...
class DomainOSINT:
    """Extended domain intelligence - DNS history, tech stack, screenshots, reputation"""
    ...
    async def securitytrails_dns_history(domain: str) -> dict:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            ...
            return {
                "domain": domain,
                "subdomains": [f"{s}.{domain}" for s in subs[:50]],
    ...
    async def urlscan(domain: str) -> dict:
        """URLScan.io - existing domain scan results"""
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            search_resp = await client.get(
                ...
                params={"q": f"domain:{domain} OR hostname:{domain}", "size": 10},
                headers=headers
            )
            if search_resp.status_code == 200:
                ...
                return {
                    "domain": domain,
                    "total_scans": search_resp.json().get("total", 0),
                    "scans": [{
                        ...
                        "malicious": r.get("verdicts", {}).get("malicious", {}).get("score", 0),
                    } for r in results]
                }
    ...
    async def urlscan_submit_and_wait(domain: str) -> dict:
        """Submit a NEW scan to URLScan and wait for the result"""
        if not settings.urlscan_api_key:
            return {"error": "No URLScan API key"}
        headers = {"API-Key": settings.urlscan_api_key, "Content-Type": "application/json"}
        async with httpx.AsyncClient(timeout=60) as client:
            submit_resp = await client.post(
                ...
                headers=headers,
                json={"url": f"https://{domain}", "public": "on"}
            )
            ...
            for _ in range(15):
                ...
                if result_resp.status_code == 200:
                    ...
                    return {
                        ...
                        "dom": f"https://urlscan.io/dom/{scan_uuid}",
                        "malicious_score": verdicts.get("malicious", {}).get("score", 0),
                    }
            ...
            if resp.status_code == 200:
                ...
                for path in groups:
                    for sub in path.get("SubDomain", []):
                        techs.append({
                            "technology": sub.get("Technology"),
                            "category": sub.get("Tag"),
                            "link": sub.get("Link"),
                        })
                seen = set()
                unique_techs = []
    ...
    async def wappalyzer_api(domain: str) -> dict:
        """Wappalyzer via API (or use self-hosted Docker CLI instead)"""
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            resp = await client.get(
                f"https://api.wappalyzer.com/v2/lookup/{domain}",
                params={"sets": "all"},
            ...
            if resp.status_code == 200:
                ...
                technologies = [{
                    "name": t.get("name"),
                    "category": [c.get("name") for c in t.get("categories", [])],
