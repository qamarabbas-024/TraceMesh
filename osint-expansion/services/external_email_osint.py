...
class EmailOSINT:
    """Extended email intelligence - breach data, reputation, verification"""

    @staticmethod
    async def hunter_verify(email: str) -> dict:
        """Hunter.io email verification + name pattern"""
    ...
    async def emailrep(email: str) -> dict:
        """EmailRep - reputation, breach, domain risk (no key required)"""
        async with httpx.AsyncClient(timeout=15) as client:
    ...
    async def dehashed_search(query: str, query_type: str = "email") -> dict:
        """Dehashed - breach data search (email, username, domain, ip)"""
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            resp = await client.get(
                ...
                params={"query": f"{query_type}:{query}", "size": 50},
                auth=auth
            )
            if resp.status_code == 200:
                data = resp.json()
    ...
    async def intelx_search(query: str) -> dict:
        """IntelX - dark web + breach intelligence"""
        ...
        async with httpx.AsyncClient(timeout=25) as client:
            search_resp = await client.post(
                ...
                json={
                    "term": query,
                    "buckets": ["leaks.public", "darknet.public"],
                    "lookuplevel": 1,
                    "maxresults": 50,
                    "timeoutsec": 10
                }
            ...
            result_resp = await client.get(
                f"https://2.intelx.io/intelligent/search/result?id={search_id}&limit=50",
                headers=headers
            )
